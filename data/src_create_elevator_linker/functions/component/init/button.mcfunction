# fail feedback
execute if entity @e[type=armor_stand, distance=..1, tag=create_elevator_linker.component] run tellraw @p [{"text": "❌ could not place button: there is already a device on this location", "color": "red"}]
...
# create button
execute align xyz positioned ~.5 ~ ~.5 unless score @p create_elevator_linker.placing_elevator_id matches 0 unless entity @e[type=armor_stand, distance=..1, tag=create_elevator_linker.component] run function src_create_elevator_linker:component/button/init
