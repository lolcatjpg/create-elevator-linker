...
execute as @e[tag=create_elevator_linker.component] at @a[scores={create_elevator_linker.placing_elevator_id = -2147483648..2147483647}] if score @s create_elevator_linker.elevator_id = @p create_elevator_linker.placing_elevator_id if score @s create_elevator_linker.floor = @p create_elevator_linker.placing_floor run effect give @s instant_health 2 1 true
