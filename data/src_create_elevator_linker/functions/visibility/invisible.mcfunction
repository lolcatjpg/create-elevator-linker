...
execute as @e[tag=create_elevator_linker.component] if data entity @s {ActiveEffects: [{ShowParticles: 0b}]} run data merge entity @s {Invisible:false, Marker:false}
execute as @e[tag=create_elevator_linker.component] unless data entity @s {ActiveEffects: [{ShowParticles: 0b}]} run data merge entity @s {Invisible:true, Marker:true}
